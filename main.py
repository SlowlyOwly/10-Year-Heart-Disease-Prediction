...
gender = col1.selectbox("Enter your gender", ["Female", "Male"])
...
# Data from survey transformation for prediction
df_pred = pd.DataFrame([[gender, age, education, isSmoker, yearsSmoking, BPMeds, stroke, hyp, diabetes, chol, sys_bp,
                         dia_bp, bmi, heart_rate, glucose]],
                       columns=['gender', 'age', 'education', 'currentSmoker', 'cigsPerDay', 'BPMeds',
...
def transform(data):
    ...
    if data == 'High school diploma':
        result = 0
    elif data == 'Undergraduate deegree':
        result = 1
    elif data == 'Postgraduate deegree':
        result = 2
    return result
...
if st.button("Predict"):
    ...
    if prediction[0] == 0:
        st.write('You likely will not develop heart disease in 10 years.')
    else:
        st.write('You are likely to develop heart disease in 10 years.')
